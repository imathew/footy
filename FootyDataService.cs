...
internal class FootyDataService : IFootyDataService
{
    ...
    public Round? FindAndParseRound(List<RoundDto> rounds, int? requestedRoundId, DateTimeOffset now)
    {
        ...
        {
            // Single pass: find first playing round and last started round simultaneously
            int playingIndex = -1;
            int lastStartedIndex = -1;

            for (int i = 0; i < rounds.Count; i++)
            {
                if (playingIndex < 0 && rounds[i].Status == "playing")
                    playingIndex = i;
            ...
            else
            {
                ...
                foundRoundIndex = (!isUnfinished && !withinCooldown && lastStartedIndex + 1 < rounds.Count)
                    ? lastStartedIndex + 1
                    : lastStartedIndex;
            }
        }

        var foundRound = rounds[foundRoundIndex];
    ...
    private static Round ConvertDtoToRound(RoundDto roundDto, DateTimeOffset now, int? prevRoundId = null, int? nextRoundId = null, string? prevRoundName = null, string? nextRoundName = null)
    {
        ...
        DateTime? firstMatchDate = matches.Count > 0 ? matches[0].Date : null;
        DateTime? lastMatchDate = matches.Count > 0 ? matches[^1].Date : null;
